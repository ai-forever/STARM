...
def stablemax_cross_entropy(logits, labels, ignore_index: int = -100):
    ...
    prediction_logprobs = torch.gather(logprobs, index=transformed_labels.to(torch.long).unsqueeze(-1), dim=-1).squeeze(
        -1)
...
class ACTLossHead(nn.Module):
    ...
    def forward(
        ...
        return new_carry, lm_loss + 0.5 * (
                q_halt_loss + q_continue_loss), metrics, detached_outputs, new_carry.halted.all()


class ACTLossHead_woq(nn.Module):
    def __init__(self, model: nn.Module, loss_type: str):
        super().__init__()
        self.model = model
        self.loss_fn = globals()[loss_type]
